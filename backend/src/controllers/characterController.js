...
async function getMyCharacter(req, res) {
  try {
    ...
    // Apply real-time AP & Credits catch-up (even if offline for hours/days)
    const regenResult = await applyRegenerationForCharacter(character.id);
    let activeChar = character;
    if (regenResult && regenResult.regenerated) {
      const { data: refreshedChar } = await supabase
        .from('characters')
        .select(`
          *,
          cities!current_city_id(name, country, continent),
        ...
        .single();
      if (refreshedChar) {
        activeChar = refreshedChar;
      }
    }

    const now = new Date();

    // Check if travel in transit has completed
    if (activeChar.travel_status === 'in_transit' && activeChar.arrives_at && new Date(activeChar.arrives_at) <= now) {
      await supabase
        .from('characters')
        .update({
          current_city_id: activeChar.destination_city_id,
      ...
      // Clear any active tags on this character as they reached their destination
      await supabase
        .from('tags')
        .delete()
      ...
      if (refreshedChar) {
        return res.json({ character: refreshedChar });
      }
    }

    // Check if surveillance mode has completed
    if (activeChar.travel_status === 'in_surveillance') {
      if (activeChar.arrives_at && new Date(activeChar.arrives_at) <= now) {
        const { completeSurveillanceForCharacter } = require('./intelController');
        const scanReport = await completeSurveillanceForCharacter(activeChar);
        ...
        if (refreshedChar) {
          refreshedChar.last_surveillance_report = scanReport;
          delete refreshedChar.player_type;
          return res.json({ character: refreshedChar });
        }
      } else {
        // Still in surveillance: parse payload and attach surveillance_info for the UI
        try {
          activeChar.surveillance_info = typeof activeChar.transport_mode === 'string'
            ? JSON.parse(activeChar.transport_mode)
            : activeChar.transport_mode;
        } catch (e) {}
      }
    }

    if (activeChar) {
      delete activeChar.player_type;
    }
    return res.json({ character: activeChar });
